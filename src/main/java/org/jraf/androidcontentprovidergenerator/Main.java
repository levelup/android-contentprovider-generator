...
public class Main {
	...
	public static class Json {
		...
		public static final String TYPE_CONSTANT_DECLARATION = "typeConstantDeclaration";
		public static final String URI_MATCHER_COMPLEMENT = "uriMatcherComplement";
		public static final String TYPE_COMPLEMENT = "typeComplement";
		public static final String GET_TYPE_COMPLEMENT = "getTypeComplement";
	}
		...
		for (File entityFile : entityFiles) {
			...
			for (int i = 0; i < len; i++) {
				...
				if (foreignKeyJson != null) {
					...
					OnDeleteAction onDeleteAction = OnDeleteAction.fromJsonName(foreignKeyJson
							.getString(Field.Json.FOREIGN_KEY_ON_DELETE_ACTION));
					foreignKey = new ForeignKey(table, onDeleteAction);
				}
				Field field = new Field(entity, name, fieldDocumentation, type, false, isIndex, isNullable, false,
						defaultValue != null ? defaultValue : defaultValueLegacy, enumName, enumValues, foreignKey);
			...
			if (idFields == null) {
				...
				if (idFields.length() != 1) {
					throw new IllegalArgumentException("Invalid number of idField '" + idFields + "' value in "
							+ entityFile.getCanonicalPath() + ".");
				}
			...
			if ("_id".equals(idFieldName)) {
				...
				if (idField == null) {
					...
					throw new IllegalArgumentException("Invalid idField: '" + idFieldName + "' not found "
							+ entityFile.getCanonicalPath() + ".");
				}
				if (idField.getType() != Field.Type.INTEGER && idField.getType() != Field.Type.LONG
						&& idField.getType() != Field.Type.DATE && idField.getType() != Field.Type.ENUM) {
					// Invalid type
					throw new IllegalArgumentException("Invalid idField type " + idField.getType() + " in "
							+ entityFile.getCanonicalPath() + "." + "  It must be Integer, Long, Date or Enum.");
				}
				if (idField.getIsNullable()) {
					// Referenced field is nullable
					throw new IllegalArgumentException("Invalid idField: '" + idFieldName + "' must not be nullable in "
							+ entityFile.getCanonicalPath() + ".");
				}
				if (!idField.getIsIndex()) {
					// Referenced field is not an index
					throw new IllegalArgumentException("Invalid idField: '" + idFieldName + "' must be an index in "
							+ entityFile.getCanonicalPath() + ".");
				}
	...
	private void validateConfig() {
		...
		if (syntaxVersion != Constants.SYNTAX_VERSION) {
			throw new IllegalArgumentException("Invalid '" + Json.SYNTAX_VERSION + "' value in _config.json: found '"
					+ syntaxVersion + "' but expected '" + Constants.SYNTAX_VERSION + "'.");
		}
		...
		ensureString(Json.TYPE_CONSTANT_DECLARATION);
		ensureString(Json.URI_MATCHER_COMPLEMENT);
		ensureString(Json.TYPE_COMPLEMENT);
		ensureString(Json.GET_TYPE_COMPLEMENT);
	}
	...
	private void ensureInt(String field) {
		try {
			...
			throw new IllegalArgumentException("Could not find '" + field
					+ "' field in _config.json, which is mandatory and must be an int.");
		}
	}
